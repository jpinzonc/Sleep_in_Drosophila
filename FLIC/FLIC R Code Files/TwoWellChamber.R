...
Feeding.TimeDependentPIPlots.DFM<-function(dfm,window.size.min=30,step.size.min=3,symbol.mult=4){
  windows(record=FALSE,width=16,height=12) # opens a window and starts recording
  op <- par(ask=FALSE)
  on.exit(par(op))
  par(mfrow=c(3,4))
  for(i in 1:6){
    Feeding.TimeDependentPIPlots(dfm,i,window.size.min,step.size.min,symbol.mult)    
  }
  windows.options(record=FALSE) #stops recording.  
}
Feeding.BoutPIPlot.DFM<-function(dfm,minEvents) {
  windows(record=FALSE,width=8,height=12) # opens a window and starts recording
  op <- par(ask=FALSE)
  on.exit(par(op))
  par(mfrow=c(3,2))
  for(i in 1:6){
    Feeding.BoutPIPlot(dfm,i,minEvents)    
  }
  windows.options(record=FALSE) #stops recording.
}
Feeding.BoutEventPIPlot.DFM<-function(dfm,minEvents) {
  windows(record=FALSE,width=8,height=12) # opens a window and starts recording
  op <- par(ask=FALSE)
  on.exit(par(op))
  par(mfrow=c(3,2))
  for(i in 1:6){
    Feeding.BoutEventPIPlot(dfm,i,minEvents)    
  }
  windows.options(record=FALSE) #stops recording.
}
##
...
TimeDependentPI.Monitors<-function(monitors,parameters,window.size.min=30,step.size.min=3,symbol.mult=4){
  ...
  for(j in monitors){
    print(paste("Analyzing Time Dependent PI for DFM ",j,".",sep=""))
    flush.console()
    monitor<-j
    x<-1:6
    dfm<-DFMClass(monitor,parameters)
    if(dfm$Parameters$Chamber.Size!=2)
      stop("This function is for two-chamber DFM only")    
    for(i in x){  
      tmp<-Feeding.TimeDependentPIPlots(dfm,i,window.size.min,step.size.min,symbol.mult=symbol.mult)
...
BoutPI.Monitors<-function(monitors,parameters,minEvents){
  ...
  for(j in monitors){
    print(paste("Analyzing BoutPI for DFM ",j,".",sep=""))
    flush.console()
    monitor<-j
    x<-1:6
    dfm<-DFMClass(monitor,parameters)
...
BoutEventPI.Monitors<-function(monitors,parameters,minEvents){
  ...
  for(j in monitors){
    print(paste("Analyzing Bout EventPI for DFM ",j,".",sep=""))
    flush.console()    
    x<-1:6
    dfm<-DFMClass(j,parameters)
    if(dfm$Parameters$Chamber.Size!=2)
      stop("This function is for two-chamber DFM only")    
    for(i in x){   
      tmp<-Feeding.BoutEventPIPlot(dfm,i,minEvents)
      num.periods<-nrow(tmp)
      tmp<-data.frame(cbind(rep(j,nrow(tmp)),rep(i,nrow(tmp)),1:nrow(tmp),tmp))
      tmp2<-matrix(rep(parameter.vector,num.periods),ncol=length(parameter.vector),byrow=TRUE)
