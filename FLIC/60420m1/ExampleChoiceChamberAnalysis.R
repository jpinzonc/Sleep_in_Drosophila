...
p<-SetParameter(p,Use.Adaptive.Threshold=F)
...
## For each of the functions below, there is either a pdf generated with some graphical representation
## of each result and/or a csv file with the numerical output.  The csv file can be loaded
## directly into MSExcel. 
Feeding.Summary.Monitors(c(1,2,3),p)
...
window.size.min=10
step.size.min=10
TimeDependentPI.Monitors(c(1,2,3),p,window.size.min,step.size.min)
...
minEvents=5
BoutPI.Monitors(c(1,2,3,4),p,minEvents)
